...
export default function FirestoreExplorer() {
    ...
    const [isDocument, setIsDocument] = useState(window.location.pathname.split('/').length % 2 !== 0)
    const [path, setPath] = useState(window.location.pathname.split('/').slice(3).join('/'))
    const windowPath = useMemo(() => window.location.pathname.split('/').slice(3), [window.location.pathname])

    // Handlers
    const handleSelectCollection = (collectionName: string) => {
        setIsDocument(false)
        navigate(`/database/${dbName}/${collectionName}`)
        setPath(`${collectionName}`)
    };
